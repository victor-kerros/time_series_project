...
``` {r import dataset}
datafile <- "gas_valeurs_mensuelles.csv"
...
Non seulement, la série intégrée d'ordre 1 ne présente pas de tendance et de constante significative mais en plus elle vérifie l’absence autocorrélation des résidus.
...
Le test ADF n'est pas rejeté, on retient son hypothèse alternative : la série est stationnaire. On détermine désormais pmax et qmax pour déterminer le modèle ARIMA.
...
ar1ma1 <- arima(logwine, c(1,1,1), optim.control=list(maxit=20000))
ma1 <- arima(logwine, c(0,1,1), optim.control=list(maxit=20000))
...
arima011 <- arima(logwine, c(0,1,1), include.mean=F)
...
``` {r forecasts}
forecasts <- forecast(ar1ma1, h=2, level=c(95))
...
require("forecast")
require("fpp2")
prev <- forecast(arima011, h=2, level=0.95)
