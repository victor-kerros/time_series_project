...
#ar9ma7 <- arima(logfertilizer, c(9,1,7), include.mean=F)
#ar7 <- arima(logfertilizer, c(7,1,0), include.mean=F)
ar9ma7 <- arima(fertilizer, c(1,1,2), include.mean=F)
ar7 <- arima(fertilizer, c(1,1,1), include.mean=F)
...
par(new=T)
plot(logfertilizer.source, type="l", lwd=1, col="blue", xlim=c(2018,2022))
```

``` {r forecasts}
prev <- forecast(ar9ma7, h=6, level=0.95)
plot(prev, xlim=c(2017,2022))
lines(dlogfertilizer.source[1:386], xlim=c(2017,2022))
```
