...
``` {r create ts clothes and dclothes with zoo}
# on permute la série sinon elle n'est pas dans le bon sens
clothes.source <- zoo(rev(data$indices), order.by=dates)
...
clothes <- clothes.source[1:(length(clothes.source)-6)]
# delec est la série en différence première
dclothes.source <- diff(clothes.source, 1)
dclothes <- diff(clothes, 1)
```

```{r plot and decompose clothes.source}
plot(clothes.source, type="l", lwd=1, col="blue", main="clothes.source", xlab="dates", ylab="values" )
decompose_clothes.source <- decompose(clothes.source)
plot(decompose_clothes.source)
```
...
logclothes.source <- log(clothes.source)
decompose_logclothes.source <- decompose(logclothes.source)
plot(decompose_logclothes.source)
plot(logclothes.source, type="l", lwd=1, col="blue", main="logclothes.source", xlab="dates", ylab="values")
```
...
```{r lm}
truncated_dates <- dates[1:(length(logclothes.source)-6)]
logclothes <- logclothes.source[1:(length(logclothes.source)-6)]
summary(lm(logclothes ~ truncated_dates))
```
...
```{r logclothes adf test}
# test ADF dans le cas avec constante et tendance
adf_logclothes <- adfTest(logclothes, lag=0, type="ct")
adf_logclothes
```
...
Qtests(adf_logclothes@test$lm$residuals, 24, length(adf_logclothes@test$lm$coefficients))
```
...
adf <- adfTest_valid(logclothes, 24, "ct")
adf
```
...
```{r dlogclothes adf test}
dlogclothes.source <- diff(logclothes.source, 1)
dlogclothes <- diff(logclothes, 1)
plot(dlogclothes.source, type="l", lwd=1, col="blue", main="dlogclothes.source", xlab="dates", ylab="values")
summary(lm(dlogclothes ~ dates[2:(length(logclothes.source)-6)]))
# il n'y a pas de tendance ni de constante
adf_dlogclothes <- adfTest(dlogclothes, lag=0, type="nc")
Qtests(adf_dlogclothes@test$lm$residuals, 24, length(adf_dlogclothes@test$lm$coefficients))
```
...
```{r dlogclothes adf test}
adf <- adfTest_valid(dlogclothes, 24, "nc")
adf
```

Le test ADF n'est pas rejeté avec seize retards pour la série différenciée, on retient son hypothèse alternative : la série est stationnaire. On détermine désormais pmax et qmax pour évaluer le modèle ARIMA(p,1,q) adéquat.

### Q3 - Représentation de la série avant et après transformation

``` {r plot clothes and dlogclothes}
plot(cbind(clothes.source,dlogclothes.source))
```
...
``` {r acf and pacf dlogclothes}
acf(dlogclothes); pacf(dlogclothes)
```
...
On ignore les autocorrélations (partielles) pour les retards supérieurs à 20 dans l'objectif d'obtenir un modèle raisonnablement simplifié. 
...
modelchoice <- function(p, q, data=dlogclothes, k=24){
...
models <- lapply(pqs, function(pq) arima(dlogclothes, c(pq[["p"]], 0, pq[["q"]])))
...
#ar9ma7 <- arima(logclothes, c(9,1,7), include.mean=F)
#ar7 <- arima(logclothes, c(7,1,0), include.mean=F)
ar9ma7 <- arima(clothes, c(1,1,2), include.mean=F)
ar7 <- arima(clothes, c(1,1,1), include.mean=F)

# on crée des séries pour les prédictions des deux modèles à tester
models <- c("ar9ma7","ar7")
preds <- zoo(matrix(NA, ncol=2, nrow=6), order.by=tail(index(clothes.source), 6))
colnames(preds) <- models
logclothesp <- preds

# on remplit la série avec les prédictions des modèles
for (model in models){
pred <- zoo(predict(get(model), 6)$pred, order.by=tail(index(clothes.source), 6))
logclothesp[, model] <- pred
}

# on affiche les valeurs observées et les prédictions
obs <- tail(clothes.source, 6)
cbind(obs, logclothesp)

# on calcule les rmse
apply(logclothesp, 2, function(x) sqrt(sum((x-obs)^2)/6)/sd(clothes.source))
```
...
tar9ma7 <- ar9ma7$coef / sqrt(diag(ar9ma7$var.coef))
pvar9ma7 <- (1-pnorm(abs(tar9ma7)))*2
pvar9ma7["ma7"]
pvar9ma7["ar9"]
Qtests(ar9ma7$residuals, 24, fitdf=1)
```
...
``` {r forecasts}
forecasts <- forecast(ar7, h=6, level=0.95)
plot(forecasts, xlim=c(2018,2022), shadecols = "grey", fcol="red", main = "intervalle de confiance (alpha = 0.95) pour T+1 et T+2")
par(new=T)
plot(logclothes.source, type="l", lwd=1, col="blue", xlim=c(2018,2022))
```

``` {r forecasts}
prev <- forecast(ar9ma7, h=6, level=0.95)
plot(prev, xlim=c(2017,2022))
lines(dlogclothes.source[1:386], xlim=c(2017,2022))
```
